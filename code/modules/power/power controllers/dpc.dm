// DPC electronics status:
/// There are no electronics in the DPC.
#define DPC_ELECTRONICS_MISSING 0
/// The electronics are installed but not secured.
#define DPC_ELECTRONICS_INSTALLED 1
/// The electronics are installed and secured.
#define DPC_ELECTRONICS_SECURED 2

// DPC cover status:
/// The DPCs cover is closed.
#define DPC_COVER_CLOSED 0
/// The DPCs cover is open.
#define DPC_COVER_OPENED 1
/// The DPCs cover is missing.
#define DPC_COVER_REMOVED 2

// DPC charging status:
/// The DPC is not charging.
#define DPC_NOT_CHARGING 0
/// The DPC is charging.
#define DPC_CHARGING 1
/// The DPC is fully charged.
#define DPC_FULLY_CHARGED 2

// DPC channel status:
/// The DPCs power channel is manually set off.
#define DPC_CHANNEL_OFF 0
/// The DPCs power channel is automatically off.
#define DPC_CHANNEL_AUTO_OFF 1
/// The DPCs power channel is manually set on.
#define DPC_CHANNEL_ON 2
/// The DPCs power channel is automatically on.
#define DPC_CHANNEL_AUTO_ON 3

// DPC autoset enums:
/// The DPC turns automated and manual power channels off.
#define AUTOSET_FORCE_OFF 0
/// The DPC turns automated power channels off.
#define AUTOSET_OFF 2
/// The DPC turns automated power channels on.
#define AUTOSET_ON 1

// External power status:
/// The DPC either isn't attached to a powernet or there is no power on the external powernet.
#define DPC_NO_POWER 0
/// The DPCs external powernet does not have enough power to charge the DPC.
#define DPC_LOW_POWER 1
/// The DPCs external powernet has enough power to charge the DPC.
#define DPC_HAS_POWER 2

// Ethereals:
/// How long it takes an ethereal to drain or charge DPCs. Also used as a spam limiter.
#define DPC_DRAIN_TIME (7.5 SECONDS)
/// How much power ethereals gain/drain from DPCs.
#define DPC_POWER_GAIN 200

// Wires & EMPs:
/// The wire value used to reset the DPCs wires after one's EMPed.
#define DPC_RESET_EMP "emp"
...
/// The bit shift for the DPCs cover status.
#define UPSTATE_COVER_SHIFT (0)
	/// The bitflag representing the DPCs cover being open for icon purposes.
	#define UPSTATE_OPENED1 (DPC_COVER_OPENED << UPSTATE_COVER_SHIFT)
	/// The bitflag representing the DPCs cover being missing for icon purposes.
	#define UPSTATE_OPENED2 (DPC_COVER_REMOVED << UPSTATE_COVER_SHIFT)

// Bitflags:
/// The DPC has a power cell.
#define UPSTATE_CELL_IN (1<<2)
/// The DPC is broken or damaged.
#define UPSTATE_BROKE (1<<3)
/// The DPC is undergoing maintenance.
#define UPSTATE_MAINT (1<<4)
/// The DPC is emagged or malfed.
#define UPSTATE_BLUESCREEN (1<<5)
/// The DPCs wires are exposed.
#define UPSTATE_WIREEXP (1<<6)

// update_overlay
// Bitflags:
/// Bitflag indicating that the DPCs operating status overlay should be shown.
#define UPOVERLAY_OPERATING (1<<0)
/// Bitflag indicating that the DPCs locked status overlay should be shown.
#define UPOVERLAY_LOCKED (1<<1)

// Bitshifts: (If you change the status values to be something other than an int or able to exceed 3 you will need to change these too)
/// Bit shift for the charging status of the DPC.
#define UPOVERLAY_CHARGING_SHIFT (2)
/// Bit shift for the equipment status of the DPC.
#define UPOVERLAY_EQUIPMENT_SHIFT (4)
/// Bit shift for the lighting channel status of the DPC.
#define UPOVERLAY_LIGHTING_SHIFT (6)
/// Bit shift for the environment channel status of the DPC.
#define UPOVERLAY_ENVIRON_SHIFT (8)



// the Departmental Power Controller (DPC)
// one per Department, needs wire connection to power network through a terminal

// controls power to devices in that department
// may be opened to change power cell
// three different channels (lighting/equipment/environ) per room - may each be set to on, off, or auto
/obj/machinery/power/apc/dpc
	...
	icon_state = "dpc-closed"
	has_electronics = DPC_ELECTRONICS_MISSING
	isDPC = TRUE
	//todo: make this figure out what rooms are part of a dept.
	var/dpcDept //what dept is this DPC in
	var/area/deptAreas = list() //list of areas managed by this DPC
	var/area/includeAreas = list() //list of areas to include in the DPC
	var/area/excludeAreas = list() //list of areas not to include in the DPC
...
/obj/machinery/power/apc/dpc/Initialize(mapload)
	...
	if(!mapload)
		return
	has_electronics = DPC_ELECTRONICS_SECURED
	...
	switch(tdir)
		if(NORTH)
			if((pixel_y != initial(pixel_y)) && (pixel_y != 23))
				log_mapping("DPC: ([src]) at [AREACOORD(src)] with dir ([tdir] | [uppertext(dir2text(tdir))]) has pixel_y value ([pixel_y] - should be 23.)")
			pixel_y = 23
		if(SOUTH)
			if((pixel_y != initial(pixel_y)) && (pixel_y != -23))
				log_mapping("DPC: ([src]) at [AREACOORD(src)] with dir ([tdir] | [uppertext(dir2text(tdir))]) has pixel_y value ([pixel_y] - should be -23.)")
		...
		if(EAST)
			if((pixel_y != initial(pixel_x)) && (pixel_x != 24))
				log_mapping("DPC: ([src]) at [AREACOORD(src)] with dir ([tdir] | [uppertext(dir2text(tdir))]) has pixel_x value ([pixel_x] - should be 24.)")
			pixel_x = 24
		if(WEST)
			if((pixel_y != initial(pixel_x)) && (pixel_x != -25))
				log_mapping("DPC: ([src]) at [AREACOORD(src)] with dir ([tdir] | [uppertext(dir2text(tdir))]) has pixel_x value ([pixel_x] - should be -25.)")
			pixel_x = -25
	if (building)
		area = get_area(src)
		opened = DPC_COVER_OPENED
		operating = FALSE
		name = "\improper [get_area_name(area, TRUE)] power controller"
...
/obj/machinery/power/apc/dpc/examine(mob/user)
	. = ..()
	if(machine_stat & BROKEN)
		return
	if(opened)
		if(has_electronics && terminal)
			. += "The cover is [opened==DPC_COVER_REMOVED?"removed":"open"] and the power cell is [ cell ? "installed" : "missing"]."
		else
	...
	. += span_notice("Alt-Click the DPC to [ locked ? "unlock" : "lock"] the interface.")

	if(issilicon(user))
		. += span_notice("Ctrl-Click the DPC to switch the breaker [ operating ? "off" : "on"].")

// update the DPC icon to show the three base states
// also add overlays for indicator lights
/obj/machinery/power/apc/dpc/update_appearance(updates=check_updates())
...
/obj/machinery/power/apc/crowbar_act(mob/user, obj/item/W)
	. = TRUE
	if (opened)
		if (has_electronics == DPC_ELECTRONICS_INSTALLED)
			if (terminal)
			...
			if(W.use_tool(src, user, 50))
				if (has_electronics == DPC_ELECTRONICS_INSTALLED)
					...
					if (machine_stat & BROKEN)
						...
						return
		else if (opened!=DPC_COVER_REMOVED)
			opened = DPC_COVER_CLOSED
...
/obj/machinery/power/apc/screwdriver_act(mob/living/user, obj/item/W)
	...
	if(opened)
		if(cell)
			...
			cell = null
			charging = DPC_NOT_CHARGING
			update_appearance()
			return
		else
			switch (has_electronics)
				if (DPC_ELECTRONICS_INSTALLED)
					has_electronics = DPC_ELECTRONICS_SECURED
					set_machine_stat(machine_stat & ~MAINT)
					W.play_tool_sound(src)
					to_chat(user, span_notice("You screw the circuit electronics into place."))
				if (DPC_ELECTRONICS_SECURED)
					has_electronics = DPC_ELECTRONICS_INSTALLED
					set_machine_stat(machine_stat | MAINT)
					W.play_tool_sound(src)
...
/obj/machinery/power/apc/welder_act(mob/living/user, obj/item/W)
	...
	if (opened && !has_electronics && !terminal)
		...
		if(W.use_tool(src, user, 50, volume=50, amount=3))
			if ((machine_stat & BROKEN) || opened==DPC_COVER_REMOVED)
...
/obj/machinery/power/apc/attackby(obj/item/W, mob/living/user, params)
	if(HAS_TRAIT(W, TRAIT_DPC_SHOCKING))
		var/metal = 0
	...
	if (istype(W, /obj/item/stock_parts/cell) && opened)
		...
		if (!(machine_stat & BROKEN || opened==DPC_COVER_REMOVED || obj_integrity < max_integrity)) // There is nothing to repair
			to_chat(user, span_warning("You found no reason for repairing this APC!"))
			return
		if (!(machine_stat & BROKEN) && opened==DPC_COVER_REMOVED) // Cover is the only thing broken, we do not need to remove elctronicks to replace cover
...
/obj/machinery/power/apc/attack_hand(mob/user, list/modifiers)
	...
	if(ishuman(user))
		...
		if(istype(maybe_stomach, /obj/item/organ/stomach/ethereal))
			var/charge_limit = ETHEREAL_CHARGE_DANGEROUS - DPC_POWER_GAIN
			...
			if((stomach?.drain_time < world.time) && LAZYACCESS(modifiers, RIGHT_CLICK))
				if(H.combat_mode)
					...
					if(stomach.crystal_charge > charge_limit)
						...
						return
					stomach.drain_time = world.time + DPC_DRAIN_TIME
					to_chat(H, span_notice("You start channeling some power through the APC into your body."))
					if(do_after(user, DPC_DRAIN_TIME, target = src))
						...
						stomach.adjust_charge(DPC_POWER_GAIN)
						cell.charge -= DPC_POWER_GAIN
					return
				else
					if(cell.charge >= cell.maxcharge - DPC_POWER_GAIN)
						to_chat(H, span_warning("The APC can't receive anymore power!"))
						return
					if(stomach.crystal_charge < DPC_POWER_GAIN)
					...
					if(do_after(user, DPC_DRAIN_TIME, target = src))
						if((cell.charge >= (cell.maxcharge - DPC_POWER_GAIN)) || (stomach.crystal_charge < DPC_POWER_GAIN))
...
/obj/machinery/power/apc/proc/update()
	if(operating && !shorted && !failure_timer)
		area.power_light = (lighting > DPC_CHANNEL_AUTO_OFF)
		area.power_equip = (equipment > DPC_CHANNEL_AUTO_OFF)
		area.power_environ = (environ > DPC_CHANNEL_AUTO_OFF)
	else
		area.power_light = FALSE
		area.power_equip = FALSE
		area.power_environ = FALSE
...
/obj/machinery/power/apc/process()
	...
	if(!avail())
		main_status = DPC_NO_POWER
	else if(excess < 0)
		main_status = DPC_LOW_POWER
	else
		main_status = DPC_HAS_POWER
	...
	if(cell && !shorted)
		...
		if(chargemode && charging == DPC_CHARGING && operating)
			if(excess > 0) // check to make sure we have enough to charge
				...
				charging = DPC_NOT_CHARGING // stop charging
				chargecount = 0

		// show cell as fully charged if so
		if(cell.charge >= cell.maxcharge)
			cell.charge = cell.maxcharge
			charging = DPC_FULLY_CHARGED
 ...
 *   - [DPC_CHANNEL_OFF]: The APCs channel has been manually set to off. This channel will not automatically change.
 *   - [DPC_CHANNEL_AUTO_OFF]: The APCs channel is running on automatic and is currently off. Can be automatically set to [DPC_CHANNEL_AUTO_ON].
 *   - [DPC_CHANNEL_ON]: The APCs channel has been manually set to on. This will be automatically changed only if the APC runs completely out of power or is disabled.
 *   - [DPC_CHANNEL_AUTO_ON]: The APCs channel is running on automatic and is currently on. Can be automatically set to [DPC_CHANNEL_AUTO_OFF].
...
/obj/machinery/power/apc/proc/autoset(val, on)
	if(on == AUTOSET_FORCE_OFF)
		if(val == DPC_CHANNEL_ON) // if on, return off
			return DPC_CHANNEL_OFF
		else if(val == DPC_CHANNEL_AUTO_ON) // if auto-on, return auto-off
			return DPC_CHANNEL_AUTO_OFF
	else if(on == AUTOSET_ON)
		if(val == DPC_CHANNEL_AUTO_OFF) // if auto-off, return auto-on
			return DPC_CHANNEL_AUTO_ON
	else if(on == AUTOSET_OFF)
		if(val == DPC_CHANNEL_AUTO_ON) // if auto-on, return auto-off
			return DPC_CHANNEL_AUTO_OFF
	return val
 ...
 *   - 1: Manually sets the APCs channel to be [DPC_CHANNEL_OFF].
 *   - 2: Manually sets the APCs channel to be [DPC_CHANNEL_AUTO_ON]. If the APC doesn't have any power this defaults to [DPC_CHANNEL_OFF] instead.
 *   - 3: Sets the APCs channel to be [DPC_CHANNEL_AUTO_ON]. If the APC doesn't have enough power this defaults to [DPC_CHANNEL_AUTO_OFF] instead.
 */
/obj/machinery/power/apc/proc/setsubsystem(val)
	if(cell && cell.charge > 0)
		return (val == 1) ? DPC_CHANNEL_OFF : val
	if(val == 3)
		return DPC_CHANNEL_AUTO_OFF
	return DPC_CHANNEL_OFF
	...
	switch(wire)
		...
		if(WIRE_AI)
			if(!wires.is_cut(WIRE_AI))
				aidisabled = FALSE
		if(DPC_RESET_EMP)
			equipment = DPC_CHANNEL_AUTO_ON
			environ = DPC_CHANNEL_AUTO_ON
			update_appearance()
			update()
...
/obj/machinery/power/apc/emp_act(severity)
	...
	if(. & EMP_PROTECT_SELF)
		return
	lighting = DPC_CHANNEL_OFF
	equipment = DPC_CHANNEL_OFF
	environ = DPC_CHANNEL_OFF
	update_appearance()
	update()
	addtimer(CALLBACK(src, .proc/reset, DPC_RESET_EMP), 600)
...
#undef DPC_CHANNEL_OFF
#undef DPC_CHANNEL_AUTO_OFF
#undef DPC_CHANNEL_ON
#undef DPC_CHANNEL_AUTO_ON
...
#undef DPC_ELECTRONICS_MISSING
#undef DPC_ELECTRONICS_INSTALLED
#undef DPC_ELECTRONICS_SECURED

#undef DPC_COVER_CLOSED
#undef DPC_COVER_OPENED
#undef DPC_COVER_REMOVED

#undef DPC_NOT_CHARGING
#undef DPC_CHARGING
#undef DPC_FULLY_CHARGED

#undef DPC_DRAIN_TIME
#undef DPC_POWER_GAIN
