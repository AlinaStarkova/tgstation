...
/obj/item/construction/factory
	name = "Factory Constructor"
	desc = "An expertly modified RCD outfitted to construct factory machinery."
	icon = 'icons/obj/tools.dmi'
	icon_state = "frcd"
	worn_icon_state = "FRCD"
	lefthand_file = 'icons/mob/inhands/equipment/tools_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/equipment/tools_righthand.dmi'
	slot_flags = ITEM_SLOT_BELT
	...
	var/list/machinery_data = list("cost" = list())
	///This list that holds all the plumbing design types the plumberer can construct. Its purpose is to make it easy to make new plumberer subtypes with a different selection of machines.
	var/list/factory_design_types

/obj/item/construction/factory/Initialize(mapload)
	. = ..()
	set_factory_designs()

/obj/item/construction/factory/attack_self(mob/user)
	..()
	if(!choices.len)
		for(var/A in factory_design_types)
			var/obj/machinery/factory/M = A

			choices += list(initial(M.name) = image(icon = initial(M.icon), icon_state = initial(M.icon_state)))
			name_to_type[initial(M.name)] = M
			machinery_data["cost"][A] = factory_design_types[A]
...
///Set the list of designs this plumbing rcd can make
/obj/item/construction/factory/proc/set_factory_designs()
	factory_design_types = list(
	/obj/machinery/factory/input = 5,
	/obj/machinery/factory/output = 5,
	/obj/machinery/factory/rack = 20,
	/obj/machinery/conveyor/factory = 2
	///obj/machinery/conveyor_switch = 1
	/*/obj/machinery/factory/acclimator = 10,
	/obj/machinery/factory/bottler = 50,
	/obj/machinery/factory/disposer = 10,
	/obj/machinery/factory/fermenter = 30,
	/obj/machinery/factory/filter = 5,
	/obj/machinery/factory/grinder_chemical = 30,
	/obj/machinery/factory/pill_press = 20,
	/obj/machinery/factory/liquid_pump = 35,
	/obj/machinery/factory/reaction_chamber = 15,
	/obj/machinery/factory/splitter = 5,
	/obj/machinery/factory/synthesizer = 15,
	/obj/machinery/factory/sender = 20
	*/
)

///pretty much rcd_create, but named differently to make myself feel less bad for copypasting from a sibling-type
/obj/item/construction/factory/proc/create_machine(atom/A, mob/user)
	if(!machinery_data || !isopenturf(A))
		return FALSE
...
/obj/item/construction/factory/proc/canPlace(turf/T)
	if(!isopenturf(T))
		return FALSE
	. = TRUE
	for(var/obj/O in T.contents)
...
/obj/item/construction/factory/afterattack(atom/A, mob/user, proximity)
	. = ..()
	if(!prox_check(proximity))
		return
	if(istype(A, /obj/machinery/factory))
		var/obj/machinery/factory/P = A
		if(P.anchored)
			to_chat(user, "<span class='warning'>The [P.name] needs to be unanchored!</span>")
			return
		if(do_after(user, 20, target = P))
