...
#define FACTORY_MAIN_MENU 1
#define FACTORY_CATEGORY_MENU 2
...
/obj/machinery/factory/Initialize()
	...
	wires = new /datum/wires/factory(src)
...
/obj/machinery/factory/ui_interact(mob/user)
	. = ..()
	if(!is_operational)
		return
	...
	switch(screen)
		if(FACTORY_MAIN_MENU)
			dat = main_win(user)
		if(FACTORY_CATEGORY_MENU)
			dat = category_win(user,selected_category)

	var/datum/browser/popup = new(user, "factory", name, 400, 500)
	popup.set_content(dat)
	popup.open()

/obj/machinery/factory/on_deconstruction()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	materials.retrieve_all()

/obj/machinery/factory/attackby(obj/item/O, mob/living/user, params)
	if (busy)
		to_chat(user, "<span class=\"alert\">The factory is busy. Please wait for completion of previous operation.</span>")
		return TRUE

	if(default_deconstruction_screwdriver(user, "factory_t", "factory", O))
		updateUsrDialog()
		return TRUE
...
/obj/machinery/factory/proc/AfterMaterialInsert(obj/item/item_inserted, id_inserted, amount_inserted)
	if(istype(item_inserted, /obj/item/stack/ore/bluespace_crystal))
		use_power(MINERAL_MATERIAL_AMOUNT / 10)
	else if(item_inserted.has_material_type(/datum/material/glass))
		flick("factory_r", src)//plays glass insertion animation by default otherwise
	else
		flick("factory_o", src)//plays metal insertion animation
...
/obj/machinery/factory/Topic(href, href_list)
	...
	if (!busy)
		...
		to_chat(usr, "<span class=\"alert\">The factory is busy. Please wait for completion of previous operation.</span>")

	updateUsrDialog()

	return

/obj/machinery/factory/proc/make_item(power, list/materials_used, list/picked_materials, multiplier, coeff, is_stack, mob/user)
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
	...
	if(is_stack)
		...
		N.update_appearance()
		N.factory_crafted(src)
	else
		for(var/i=1, i<=multiplier, i++)
			var/obj/item/new_item = new being_built.build_path(A)
			new_item.factory_crafted(src)
...
/obj/machinery/factory/RefreshParts()
	var/mat_capacity = 0
...
/obj/machinery/factory/proc/main_win(mob/user)
	var/dat = "<div class='statusDisplay'><h3>factory Menu:</h3><br>"
	...
	for(var/C in categories)
		...
		dat += "<td><A href='?src=[REF(src)];category=[C];menu=[FACTORY_CATEGORY_MENU]'>[C]</A></td>"
		line_length++

	dat += "</tr></table></div>"
	return dat

/obj/machinery/factory/proc/category_win(mob/user,selected_category)
	var/dat = "<A href='?src=[REF(src)];menu=[FACTORY_MAIN_MENU]'>Return to main menu</A>"
...
/obj/machinery/factory/proc/materials_printout()
	var/datum/component/material_container/materials = GetComponent(/datum/component/material_container)
...
/obj/machinery/factory/proc/can_build(datum/design/D, amount = 1)
	if(D.make_reagents.len)
		return FALSE

	var/coeff = (ispath(D.build_path, /obj/item/stack) ? 1 : 1)
...
/obj/machinery/factory/proc/get_design_cost(datum/design/D)
	var/coeff = (ispath(D.build_path, /obj/item/stack) ? 1 : 1)
	var/dat
	for(var/i in D.materials)
...
/obj/machinery/factory/proc/reset(wire)
	switch(wire)
		if(WIRE_HACK)
...
/obj/machinery/factory/proc/adjust_hacked(state)
	hacked = state
	for(var/id in SSresearch.techweb_designs)
		var/datum/design/D = SSresearch.techweb_design_by_id(id)
		if((D.build_type & FACTORY) && ("hacked" in D.category))
			if(hacked)
				stored_research.add_design(D)
			else
				stored_research.remove_design(D)

/obj/machinery/factory/hacked/Initialize()
	. = ..()
	adjust_hacked(TRUE)

/obj/item/proc/factory_crafted(obj/machinery/factory/A)
	return

/obj/machinery/factory/input
	name = "Input Chute"
	desc = "A chute that leads into a machine."
	icon_state = "input"
	idle_power_usage = 5
	active_power_usage = 10

	circuit = /obj/item/circuitboard/machine/factory/input
	density = FALSE

/obj/machinery/factory/input/ui_interact(mob/user)
	return

/obj/machinery/factory/input/attackby(obj/item/O, mob/living/user, params)
	if (busy)
		to_chat(user, "<span class=\"alert\">The factory input is busy. Please wait for completion of previous operation.</span>")
		return TRUE

	if(default_deconstruction_screwdriver(user, "input_t", "input", O))
		updateUsrDialog()
		return TRUE
...
/obj/machinery/factory/output
	name = "Output Chute"
	desc = "A chute that leads out of a machine."
	icon_state = "output"
	idle_power_usage = 5
	active_power_usage = 10

	circuit = /obj/item/circuitboard/machine/factory/output
	density = FALSE

/obj/machinery/factory/output/ui_interact(mob/user)
	return

/obj/machinery/factory/output/attackby(obj/item/O, mob/living/user, params)
	if (busy)
		to_chat(user, "<span class=\"alert\">The factory output is busy. Please wait for completion of previous operation.</span>")
		return TRUE

	if(default_deconstruction_screwdriver(user, "output_t", "output", O))
		updateUsrDialog()
		return TRUE
...
/obj/machinery/factory/metal_shaper
	name = "Metal Shaper"
	desc = "Able to shape raw sheets of metal into metal products."
	icon_state = "factory"

	circuit = /obj/item/circuitboard/machine/factory/metal_shaper
